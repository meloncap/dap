...
import CreateStake, { Locking, } from "../components/Stake/CreateStake";
...
const LockingPage = () => {
  ...
  const loadUserInfo = async () => {
    ...
    for (let i = 0; i < stakeInfo.stakedInfo.length; i++) {
      if (
        Number(stakeInfo.stakedInfo[i].duration) > 0 &&
        ...
        const canUnstake = await isWidthdraw(stakeInfo.stakedInfo[i].name, account);
        ...
        tmp.push({
          ...
          canUnstake,
          duration: stakeInfo.stakedInfo[i].duration,
        });
      }
    }
  ...
  return (
    <PageContainer>
      ...
      <Box mb="50px">
        <EnhancedTable rows={userStakeInfo} onSelected={selected} onUnstake={loadUserInfo}/>
      </Box>
    </PageContainer>
  );
};
...
function EnhancedTable({ rows, onSelected, onUnstake }) {
  ...
  const unStake = (name) => async(e) => {
    if (!account) return;
    e.stopPropagation();
    await toast.promise(withdraw(name, account), {
