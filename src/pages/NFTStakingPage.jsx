...
import NFTCard, {NFTCardContainer} from "../components/NFTCard";
...
const NFTStakingPage = () => {
  ...
  const [useNFT, setUserNFT] = useState([])
  ...
  const loadUserInfo = async () => {
    ...
    for (let i = 0; i < stakeInfo.stakedInfo.length; i++) {
      if ( Number(stakeInfo.stakedInfo[i].duration) === 0 &&
            Number(stakeInfo.stakedInfo[i].StakeNFTId) !== 0
      ) {
        ...
        const uri = await getStakedTokenURI(stakeInfo.stakedInfo[i].StakeNFTId)
    ...
    const nftInfo = await getUserNFT(account)
    setUserNFT(nftInfo)
  ...
  return (
    <PageContainer>
      <Title title="NFT Staking" mb="50px" />
      <ChartWraper mb="50px">
        <AChart
          className="col1"
      ...
      <Box 
        ...
        height={useNFT.length ? '555px' : '250px'}
      >
        <Box component="h5" my="10px">Your NFT</Box>
        <NFTCardContainer >
          {_.map(useNFT, (each, index)=>{
            return <NFTCard tokenId={each.tokenId} uri={each.uri} key={index} close={toggle !== each.tokenId} onToggle={(id)=>{setToggle(id)}} staked={loadUserInfo} mr="10px" />
          })}
        </NFTCardContainer>
      </Box>
      ...
      <Box mb="50px">
        <EnhancedTable rows={userStakeInfo} onSelected={selected} viewNFT={loadNFT} onUnstake={loadUserInfo} />
      </Box>
...
function EnhancedTable({ rows, onSelected, viewNFT, onUnstake }) {
  ...
  const unStake = (name) => async(e) => {
    if (!account) return;
    e.stopPropagation();
    await toast.promise(UnStakeNFT(account, name), {
      pending: "Unstaking Now...",
      success: "Success!",
      error: "Something went wrong!",
    });
    onUnstake()
